...
def ltb_luminosity_distance(z_arr):
    ...
    for zi in np.atleast_1d(z_arr):
        ...
        dzs  = np.diff(zs)
        H_lz = E1_H_MEAN + E1_DELTA_H * np.exp(-zs[:-1]**2 / (2.0 * E1_SIGMA_Z**2))
...
# ── Phase 2: Rebound — pressure builds, E(r) flips ────────────
# Show R staying near R_min over a brief time interval (qualitative)
eta_p2_brief = np.linspace(eta_min - 0.15, eta_min, 60)
R_p2   = 1.0 - np.cos(eta_p2_brief)
t_p2   = eta_p2_brief - np.sin(eta_p2_brief)
# Extend time axis slightly for visibility
t_p2_disp = np.linspace(t_p2[0], t_p2[-1] + 0.4, 60)
R_p2_disp = np.where(t_p2_disp <= t_p2[-1],
                     np.interp(t_p2_disp, t_p2, R_p2),
                     R_min_abs)
...
for idx, (ax, (title, t_ph, R_ph, col, key)) in \
    ...
    if key == 'collapse':
        ...
        ax.set_xlim(t_p2_disp[0] - 0.2, t_p2_disp[-1] + 0.1)
...
# Place r_schwarz so that r_cross / r_schwarz ≈ Engine 5 ratio = 2.072
# The crossover r_cross is found numerically:
cross_idx = np.argmin(np.abs(a_total - a_grav))
r_cross   = r_core[cross_idx]
# r_schwarz is set to r_cross / E5_RATIO to reproduce the Engine 5 ratio
